...
install_mikrotik_docker() {
    ...
    if [ "$add_ports" == "y" ]; then
        ...
        docker run --cap-add=NET_ADMIN --device=/dev/net/tun -d --name livekadeh_com_mikrotik7_7 --restart unless-stopped -p 8291:8291 -p 80:80 $port_mappings -ti livekadeh_com_mikrotik7_7
