...
install_mikrotik_docker() {
    ...
    if [ "$add_ports" == "y" ]; then
        ...
        docker run --cap-add=NET_ADMIN --device=/dev/net/tun -d --name livekadeh_com_mikrotik7_7 --restart always -p 8291:8291 -p 80:80 $port_mappings -ti livekadeh_com_mikrotik7_7
        ...
        docker run --cap-add=NET_ADMIN --device=/dev/net/tun -d --name livekadeh_com_mikrotik7_7 --restart always -p 8291:8291 -p 80:80 -ti livekadeh_com_mikrotik7_7
        docker attach livekadeh_com_mikrotik7_7
    fi
}
...
menu() {
    clear
    echo "By --> iPmartNetwork * Github.com/iPmartNetwork * "
    echo ""
